...
import com.xxw.base.util.CollectionUtil;
import com.xxw.base.util.SqlUtil;
import com.xxw.base.util.StringUtils;
...
public class Condition {
    ...
    public String toString(List<Object> paramValues) {
        final StringBuilder conditionStrBuilder = StringUtils.builder();
        // 判空值
        checkValueNull();
    ...
    private void buildValuePartForBETWEEN(StringBuilder conditionStrBuilder, List<Object> paramValues) {
        ...
        // 处理 AND y
        conditionStrBuilder.append(" ").append("and");
        if (isPlaceHolder()) {
            // 使用条件表达式占位符
            conditionStrBuilder.append(" ?");
            if (null != paramValues) {
    ...
    private void parseValue() {
        ...
        // 对数组和集合值按照 IN 处理
        if (this.value instanceof Collection || CollectionUtil.isArray(this.value)) {
        ...
        List<String> strs = StringUtils.split(valueStr, ' ', 2);
        if (strs.size() < 2) {
            return;
        }
        ...
        if (OPERATOR_BETWEEN.equals(firstPart)) {
            final List<String> betweenValueStrs = StringUtils.splitTrimIgnoreCase(strs.get(1), "and", 2, true);
